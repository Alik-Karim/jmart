...
import com.AbdulMalikKarimAJmartMR.*;
...
public class PaymentController implements BasicGetController<Payment> {
    ...
    @JsonAutowired(value = Payment.class, filepath = "D:/project/jmart/src/main/java/com/AbdulMalikKarimAJmartMR/payment.json")
    public static JsonTable<Payment> paymentTable;

    public static ObjectPoolThread<Payment> poolThread;
    static {
        poolThread = new ObjectPoolThread<>("message", PaymentController::timekeeper);
        poolThread.start();
    }
            ...
            )
    {
        ...
        Payment.Record record = payment.history.get(payment.history.size() - 1);
        if(record.status != Invoice.Status.WAITING_CONFIRMATION) return false;
        payment.history.add(new Payment.Record(Invoice.Status.CANCELLED, "message"));
        return true;
    }
            ...
            )
    {
        ...
        payment.history.add(new Payment.Record(Invoice.Status.ON_DELIVERY, "message"));
        return true;
    }
    ...
    private static boolean timekeeper(Payment payment) {
        ...
        if(record.status == Invoice.Status.WAITING_CONFIRMATION && elapsed > WAITING_CONF_LIMIT_MS) {
            payment.history.add(new Payment.Record(Invoice.Status.FAILED, "message"));
            return true;
            ...
            payment.history.add(new Payment.Record(Invoice.Status.DELIVERED, "message"));
            return false;
        } else if(record.status == Invoice.Status.DELIVERED && elapsed > DELIVERED_LIMIT_MS) {
            payment.history.add(new Payment.Record(Invoice.Status.FINISHED, "message"));
            return true;
        }
        return false;
    }
